...
public static function showTables() {
    global $lng, $tours, $get_lid;
    title($lng->getTrn('name', 'LeagueTables'));

	// Selector for the tournament
	$lid = $_SESSION["NS_node_id"];
    $tour_id = get_alt_col('league_prefs','f_lid',$lid,'prime_tid');
...
/*
	...
	echo "-->\n";
	*/
	// Hard coded list of fields to show - matching existing league tables
	$fields = array(
		...
		if ($allowedTeams == 0 || in_array($t['team_id'], $allowedTeams)) {
			...
			foreach ($fields as $field) {
				...
				if ($field == 'tv') {
					...
					$value = " width='30%'><a href='". urlcompile(T_URL_PROFILE,T_OBJ_TEAM,$t['team_id'],false,false)."'>$t[$field]</a>";
				} else if ($field == 'f_cname') {
					$value = " width='18%'><a href='". urlcompile(T_URL_PROFILE,T_OBJ_COACH,$t['owned_by_coach_id'],false,false) ."'>$t[$field]</a>";
				} else if ($field == 'f_rname') {
					$value = " width='18%'><a href='". urlcompile(T_URL_PROFILE,T_OBJ_RACE,$t['f_race_id'],false,false) ."'>$t[$field]</a>";
				} else {
					$value = ">".$value;
				}
				echo "<td" . $value  . "</td>";
