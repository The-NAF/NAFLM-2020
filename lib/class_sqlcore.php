...
class SQLCore
{
	...
	*/
	public static function syncGameData() {
		global $core_tables, $DEA, $stars, $skillarray, $specialrulesarray ;
		$players   = 'game_data_players';
		$races     = 'races';
		$starstbl  = 'game_data_stars';
		$skillstbl = 'game_data_skills';
		$specialrulestbl = 'special_rules';
		$status = true;
		...
		foreach ($specialrulesarray as $cat => $rules) {
			foreach ($rules as $rid => $r) {
				$status &= mysql_query("INSERT INTO $specialrulestbl(rule_id, name, cat) VALUES ($rid, '".mysql_real_escape_string($r)."', '$cat')");
			}
		}
	...
	public static function installProcsAndFuncs($install = true) {
		 ...
		 */
		// MV syncs
		$common_fields_keys          = 'td,cp,deflct,intcpt,bh,si,ki,misc,mvp,cas,tdcas,spp';
		$common_fields               = 'IFNULL(SUM(td),0), IFNULL(SUM(cp),0), IFNULL(SUM(deflct),0), IFNULL(SUM(intcpt),0), IFNULL(SUM(bh),0), IFNULL(SUM(si),0), IFNULL(SUM(ki),0), IFNULL(SUM(misc),0), IFNULL(SUM(mvp),0), IFNULL(SUM(bh+si+ki),0), IFNULL(SUM(bh+si+ki+td),0), IFNULL(SUM((cp+deflct+misc)*1+(bh+si+ki)*2+intcpt*2+td*3+mvp*4),0)';
		...
		$matches_setup_rels = '
			...
			DECLARE inj_ma,inj_av,inj_ag,inj_pa,inj_st,inj_ni, ma,av,ag,pa,st '.$CT_cols['chr'].';
			DECLARE ma_ua,av_ua,ag_ua,pa_ua,st_ua '.$CT_cols['chr_ua'].';
		...
		$matches_player_all_stats = '
			...
			REPEAT
				...
				IF NOT done THEN
					...
					CALL getPlayerDProps(pid, inj_ma,inj_av,inj_ag,inj_pa,inj_st,inj_ni, ma,av,ag,pa,st, ma_ua,av_ua,ag_ua,pa_ua,st_ua, value,status,date_died);
					UPDATE players
		...
		$routines = array(
			...
			'CREATE PROCEDURE getPlayerDProps(
				...
				OUT inj_ma '.$CT_cols['chr'].',   OUT inj_av '.$CT_cols['chr'].',   OUT inj_ag '.$CT_cols['chr'].',   OUT inj_pa '.$CT_cols['chr'].',   OUT inj_st '.$CT_cols['chr'].', OUT inj_ni '.$CT_cols['chr'].',
				OUT ma '.$CT_cols['chr'].',       OUT av '.$CT_cols['chr'].',       OUT ag '.$CT_cols['chr'].',       OUT pa '.$CT_cols['chr'].',       OUT st '.$CT_cols['chr'].',
				OUT ma_ua '.$CT_cols['chr_ua'].', OUT av_ua '.$CT_cols['chr_ua'].', OUT ag_ua '.$CT_cols['chr_ua'].', OUT pa_ua '.$CT_cols['chr_ua'].', OUT st_ua '.$CT_cols['chr_ua'].',
			...
			BEGIN
				...
				INTO
					f_pos_id, extra_val, ach_ma, ach_st, ach_ag, ach_pa, ach_av
				...
				SELECT
					...
					IFNULL(SUM(IF(inj = '.PA.', 1, 0) + IF(agn1 = '.PA.', 1, 0) + IF(agn2 = '.PA.', 1, 0)), 0),
				...
				SET value = (SELECT cost FROM game_data_players WHERE game_data_players.pos_id = f_pos_id)
					...
					- inj_pa * '.$rules['value_reduction_pa'].'
				...
				SELECT
					game_data_players.ma, game_data_players.st, game_data_players.ag, game_data_players.pa, game_data_players.av
				INTO
					def_ma,def_st,def_ag,def_pa,def_av
				...
				SET ag_ua = CONVERT(def_ag - ach_ag,SIGNED) + CONVERT(inj_ag,SIGNED);
				SET pa_ua = (CASE WHEN def_pa = 0 AND ach_pa > 0 THEN CONVERT(7 - ach_pa,SIGNED) + CONVERT(inj_pa,SIGNED)
				ELSE CONVERT(def_pa - ach_pa,SIGNED) + CONVERT(inj_pa,SIGNED) END);
				...
				SET pa = calcEffectiveChr(def_pa, pa_ua);
			...
			BEGIN
				...
				IF def > 0 THEN
					...
					IF diff >= 0 THEN
						RETURN IF(ua > 11, 11, IF(diff <= 2, ua, def+2));
					ELSE
				...
				ELSE 
					SET diff = CONVERT(ua,SIGNED) - CONVERT(def,SIGNED);
					IF diff >= 5 THEN
						RETURN IF(ua > 6, 6, IF(diff > 4, ua, 5));
					ELSE
						RETURN 0;
					END IF;
				END IF;
