    ...
    protected static function _player($T_SQLTRIG, array $argv) {
        ...
        switch ($T_SQLTRIG) {
            ...
            case T_SQLTRIG_PLAYER_DPROPS: 
                mysql_query("CALL getPlayerDProps($pid, @inj_ma,@inj_av,@inj_ag,@inj_pa,@inj_st,@inj_ni, @ma,@av,@ag,@pa,@st, @ma_ua,@av_ua,@ag_ua,@pa_ua,@st_ua, @value, @status, @date_died)") or die(mysql_error());
                mysql_query("UPDATE players SET 
                            inj_ma = @inj_ma, inj_av = @inj_av, inj_ag = @inj_ag, inj_pa = @inj_pa, inj_st = @inj_st, inj_ni = @inj_ni, 
                            ma = @ma, av = @av, ag = @ag, pa = @pa, st = @st, 
                            ma_ua = @ma_ua, av_ua = @av_ua, ag_ua = @ag_ua, pa_ua = @pa_ua, st_ua = @st_ua, 
