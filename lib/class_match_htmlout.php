...
$T_MOUT_ACH = array_combine($T_PMD_ACH, array('MVP','Cp','Td','Def','Int','BH','SI','Ki','Misc SPP',));
		...
		if (isset($_POST['button']) && $ALLOW_EDIT) {
			...
			foreach (array(1 => $team1, 2 => $team2) as $id => $t) {
				if (in_array($t->f_race_id, $racesHasNecromancer) && isset($_POST["t${id}zombie"])) {
					$pos_id = $DEA[$t->f_rname]['players']['Zombie Lineman']['pos_id'];
					...
					/*
						Knowing the new zombie's PID we relocate the zombie match data to regular player data - this allows us to use the same loop for submitting the zombie's match data.
					*/
				...
				if (in_array($t->f_race_id, $racesMayRaiseRotters) && isset($_POST["t${id}rotter"]))  {
						...
						list($exitStatus,$pid) = Player::create(
							array(
								...
								'f_pos_id' => $pos_id,
								'team_id' => $t->team_id,
								'name' => "RAISED ROTTER JOURNEYMAN"
							),
							array(
								'JM' => true,'free' => true,'force' => true,	
							));
						/*
							Knowing the new rotter's PID we relocate the rotter match data to regular player data - this allows us to use the same loop for submitting the rotter's match data.
						*/
						foreach ($T_PMD__ENTRY_EXPECTED as $f) {
				...
				foreach ($stars as $star) {
					...
					if (isset($_POST['team_'.$star['id']]) && $_POST['team_'.$star['id']] == $id) {
						...
						$m->entry($sid, array(
							...
							'cp'      => $_POST["cp_$sid"],
							'td'      => $_POST["td_$sid"],
							'deflct'  => $_POST["deflct_$sid"],
							'intcpt'  => $_POST["intcpt_$sid"],
							'bh'      => $_POST["bh_$sid"],
							'si'      => $_POST["si_$sid"],
							'ki'      => $_POST["ki_$sid"],
							'misc'    => $_POST["misc_$sid"],
							'ir1_d1'  => 0,
							'ir1_d2'  => 0,
							'ir2_d1'  => 0,
							'ir2_d2'  => 0,
